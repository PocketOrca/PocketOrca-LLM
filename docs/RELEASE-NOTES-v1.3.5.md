...
- **品牌更名**：AnNPU-LM → PocketOrca-LLM（App 显示名、图标、全部文档；包名 `com.dawoer.npullm` 不变，已装用户无感）
- **全新图标**：鲸鱼主题，四轮迭代（构图放大 → 居中修正 → 内容再放大 → 字体重制与取景修正）
- 正式发布签名（RSA4096，有效期至 2056 年）

### v1.3.1 关键修复（继承）

- **息屏 / 滑卡后服务断连**：上滑清理会杀死整个应用进程（`am_kill remove task`），前台服务无法拦截。v1.3.1 增加三层防护：
